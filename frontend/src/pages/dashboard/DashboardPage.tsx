  ...
  return (
    <div className="flex flex-col flex-1 space-y-4 min-h-0">
      ...
      <Dialog open={aiOpen} onOpenChange={setAiOpen}><DialogContent className={aiStep === "preview" ? "sm:max-w-3xl" : "sm:max-w-lg"}><DialogHeader><DialogTitle className="flex items-center gap-2"><Sparkles className="h-5 w-5" />Generate Folders with AI</DialogTitle><DialogDescription>Describe how your organization works and AI will suggest a folder tree.</DialogDescription></DialogHeader>
        ...
        {aiStep === "preview" && (<div className="space-y-4"><div className="rounded-lg border overflow-hidden bg-muted/20 h-[280px]"><ReactFlow nodes={aiFlow.nodes} edges={aiFlow.edges} nodeTypes={aiNodeTypes} defaultEdgeOptions={{ type: "step", style: { stroke: "#818cf8", strokeWidth: 1.5 } }} fitView fitViewOptions={{ padding: 0.4 }} nodesDraggable={false} nodesConnectable={false} elementsSelectable={false} panOnDrag={false} zoomOnScroll={false} zoomOnDoubleClick={false} proOptions={{ hideAttribution: true }} /></div><p className="text-xs text-muted-foreground text-center">This is a sample preview based on your description.</p><div className="flex justify-end gap-2"><Button variant="ghost" onClick={() => setAiStep("prompt")}>Back</Button><Button onClick={() => { if (!orgId || !aiResult) return; applyFolders.mutate({ orgId, roots: extractNewRoots(aiResult.roots) }, { onSuccess: () => setAiStep("done") }) }} disabled={applyFolders.isPending}>{applyFolders.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}Create Folders</Button></div></div>)}
